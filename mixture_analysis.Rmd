...
results2i <- gwqs(formula = CPC_lm_residuals~ wqs*childs_sex + mom_age_at_birth + mom_education + MomBMI_36wks +  childs_sex + MomBMI_10wks + official_enroll_category, bint_cont_pos = TRUE, mix_name = PFASp, data = pheno, q = 10, validation = 0, b = 20, b1_pos = T, plan_strategy = "multicore", family = "gaussian", seed = 10)

summary(results2i)
```
```{r}
PFASp = c("PFOA","PFHxS","PFNA","PFDA","PFOS")
results2i <- gwqs(formula = CPC_lm_residuals~ wqs + mom_age_at_birth + mom_education + MomBMI_36wks  + MomBMI_10wks + official_enroll_category, bint_cont_pos = TRUE, mix_name = PFASp, data = pheno[which(pheno$childs_sex == 1),], q = 10, validation = 0, b = 20, b1_pos = T, plan_strategy = "multicore", family = "gaussian", seed = 10)
